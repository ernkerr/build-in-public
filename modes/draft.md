...
1. Read `config/profile.yml` for available platforms and the user's identity/handles
2. Read `data/voice/never-words.md` and `data/voice/non-negotiables.md` (hard prohibitions) + their `data/local/voice/` mirrors if present
3. Read `data/voice/voice.md` (universal voice direction) + `data/local/voice/voice.md` if present
...
1. Read `data/voice/{primary-platform}/voice.md` for platform direction (rhythm, format constraints, voice notes) + the `data/local/voice/` mirror if present
2. If the task is task-specific (e.g. an Instagram Reel scripting pass): also read `data/voice/{primary-platform}/{task}.md` + local mirror
3. Read `data/local/references/{primary-platform}/*.md` for structural patterns and what's worked before
...
- [ ] No banned vocabulary or cadences from `data/voice/never-words.md` (+ local mirror)
- [ ] No violations of `data/voice/non-negotiables.md` (+ local mirror) — em dashes, "It's not X. It's Y.", bait closes
...
- [ ] Reads like the author under that platform's constraint — not like the platform stereotype
- [ ] Personal punctuation tells (from `data/local/voice/`) preserved exactly, never normalized
- [ ] Single tasteful emoji at end, max — strip any extras
- [ ] Structurally matches a pattern from `data/local/references/{platform}/*.md` (or is a deliberate departure)
...
1. Read `data/voice/{platform}/voice.md` (+ local mirror) and `data/local/references/{platform}/*.md` for that platform (universal voice already loaded from Step 2)
...
Save to `data/local/drafts.md`. Add a new row per platform:
