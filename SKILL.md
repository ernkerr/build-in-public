...
Hard prohibitions ship as base defaults — [data/voice/never-words.md](data/voice/never-words.md) and [data/voice/non-negotiables.md](data/voice/non-negotiables.md) — extended by personal mirrors in `data/local/voice/` (gitignored). All are read on-demand via `voice-lookup.md` to keep root context lean. Base = what a fork inherits; personal = this user's voice.
