...
- Voice is two layers, always read base then personal: `data/voice/...` (committed defaults) + `data/local/voice/...` (gitignored personal mirror)
- `data/voice/never-words.md` + `data/voice/non-negotiables.md` (+ personal mirrors) — hard prohibitions (always read first)
- `data/voice/voice.md` (+ mirror) — universal voice direction
- `data/voice/{platform}/voice.md` (+ mirror) — per-platform voice (read after primary platform is picked)
- `data/voice/{platform}/{task}.md` (+ mirror) — per-task voice within a platform (e.g. `instagram/scripting.md`)
- `data/local/references/{platform}/*.md` — the user's saved inspiration per platform
- `data/local/audits/{platform}/*.md` — audits of the user's own published posts
- `data/local/drafts.md` — current draft queue
- `data/local/published.md` — publication history
